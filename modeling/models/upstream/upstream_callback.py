...
class UpstreamCallback:
    ...
    def __init__(self, server_callbacks: list[ServerCallback]) -> None:
        self.complete_factor_deviation = UpstreamCallback.solve_average_deviation([callback.complete_factor for callback in server_callbacks])
        self.downtime_factor_deviation = UpstreamCallback.solve_average_deviation([callback.downtime_factor for callback in server_callbacks])
        self.overload_factor_deviation = UpstreamCallback.solve_average_deviation([callback.overload_factor for callback in server_callbacks])
